...
import { ArrowRight, BarChart3, CalendarClock, Cpu, Users2 } from 'lucide-react'

import { Container } from '@/components/layout/container'
import { Button } from '@/components/ui/button'

const platformPreviewRows = [
  { label: 'Projetos publicados', value: '—' },
  { label: 'Equipes formadas', value: '—' },
  { label: 'Solicitações de participação', value: '—' },
] as const
          ...
          >
            ...
            <div className="space-y-6">
              ...
              <p className="text-balance text-lg text-white/80 sm:text-xl">
                Publique projetos, encontre colaboradores e forme equipes acadêmicas para transformar ideias em soluções reais —
                com método, tecnologia e transparência.
              </p>
            </div>
            ...
            <div className="grid gap-3 sm:grid-cols-3">
              {[
                { icon: Cpu, label: 'Tecnologia aberta para equipes multidisciplinares' },
                { icon: Users2, label: 'Colaboração com papéis, vagas e habilidades visíveis' },
                { icon: CalendarClock, label: 'Pronto para evoluir com banco de dados e API institucional' },
              ].map(({ icon: Icon, label }) => (
                <div
                  key={label}
                  className="flex gap-3 rounded-2xl border border-white/20 bg-white/5 p-4 text-left backdrop-blur"
                >
          ...
          >
            ...
            <div className="relative space-y-6 rounded-[2rem] border border-white/15 bg-white/5 p-6 shadow-2xl backdrop-blur-xl">
              <div className="flex items-start gap-3">
                ...
                <div>
                  <p className="text-sm font-semibold text-white">Visão da plataforma</p>
                  <p className="mt-1 text-sm text-white/70">
                    Métricas reais aparecerão aqui após integração com o banco — sem números inventados até lá.
                  </p>
                </div>
              </div>
              <ul className="space-y-3 border-t border-white/15 pt-4">
                {platformPreviewRows.map((row) => (
                  ...
                  >
                    ...
                    <span className="text-xl font-semibold tabular-nums text-white">{row.value}</span>
                  </li>
                ))}
              </ul>
            </div>
